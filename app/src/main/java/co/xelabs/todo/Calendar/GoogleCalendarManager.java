...
import co.xelabs.todo.Calendar.Tasks.DeleteCalendarItemTask;
import co.xelabs.todo.Calendar.Tasks.GetCalendarItemsTask;
import co.xelabs.todo.Calendar.Tasks.PostCalendarItemTask;
import co.xelabs.todo.Calendar.Tasks.UpdateCalendarItemTask;
...
public class GoogleCalendarManager {
    ...
    /**
     * Sets up the necessary permissions and credentials to use the Google Calendar api, then updates an event within the specified calendar.
     * NOTE: The calling activity needs to call handleOnActivityResult inside onActivityResult in order to handle navigation to the google sign in activity
     *
     * @param activity - activity to display google account related UI on.
     * @param calendarAware - class that contains callbacks for when async processing concludes
     * @param calendarName - name of the calendar
     * @param oldName - event name before the update
     * @param calendarEvent - contains the updated version of the calendar event
     */
    public synchronized void updateCalendarItem(Activity activity, CalendarAware calendarAware, String calendarName, String oldName, CalendarEvent calendarEvent){
        currOp = new UpdateCalendarItemTask(mCredential, activity, calendarAware, calendarName, oldName, calendarEvent);
        makeApiCall(activity);
    }
     ...
     */
    public synchronized void deleteCalendarItem(Activity activity, CalendarAware calendarAware, String calendarName, String eventId){
        currOp = new DeleteCalendarItemTask(mCredential, activity, calendarAware, calendarName, eventId);
        makeApiCall(activity);
    }
